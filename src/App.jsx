...
function Home() {
  ...
  const [names, setNames] = useState(
    rotateArray(["B", "Mama", "Parthe", "Muno"], differenceInWeeks)
  );
  ...
  useEffect(() => {
    fetch(apiUrl, {
      ...
      .then((response) => {
        if (!response.ok) {
          setErrorMessage(
            `Problem Fetching Data... if messages appears for more than 4 seconds check Network and reload.`
          );
      ...
      .catch((error) => {
        ...
        if (error instanceof TypeError && error.message === "Failed to fetch") {
          setErrorMessage(
            "Check your network connection or Api is down, try again later"
          ); // Additional handling
          return;
        } else if (
          ...
          setErrorMessage(
            `Fetching new book please wait... if messages appears for more than 4 seconds check Network and reload.`
          );
        }
      });
    ...
    if (bible && bible.content) {
      ...
      if (verses) {
        ...
        for (let i = 0; i < numPartitions; i++) {
          console.log(
            `Partition ${i + 1}: Verses ${verseRanges[i].start}-${
              verseRanges[i].end
            }`
          );
        }
